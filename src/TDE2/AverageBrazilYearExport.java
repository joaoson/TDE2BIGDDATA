...
public class AverageBrazilYearExport {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Path output = new Path("output/Question7TDE.txt");
        Job job = Job.getInstance(conf, "AverageBrazil");

        job.setJarByClass(AverageBrazilYearExport.class);
        ...
        if (job.waitForCompletion(true)) {
            ...
            Path outputFile = new Path("output/Question7TDETEXT.txt");
    ...
    public static class AverageBrazilMapper extends Mapper<LongWritable, Text, Text, ValueCountWritable> {
        ...
        public void map(LongWritable key, Text value, Context context)
            ...
            if (fields.length <11) {
                if (fields[0].equals("Brazil") && fields[4].equals("Export")) {
                    try {
                        word.set(fields[1]);
